...
use common::{generate_icechunk_store_local, run_memory_profile, run_bench, ArraysToGenerate, BBOX_SQL};
...
fn bbox_bench_local(c: &mut Criterion) {
    ...
    let (session, _temp_dir) = generate_icechunk_store_local(&rt, ArraysToGenerate::BboxOnly).unwrap();
